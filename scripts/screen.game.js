  ...
  this.multiply = function () {
	...
	//Touch Events
		$(window).load (function (e) {
		 $("#up, #left, #right, #fire, #pause").bind("touchstart touchend",function(e){
		   var touchaction = e.target.id
		   if (e.type == "touchstart") {
			if (touchaction == "up"){
			 touchaction = "up";
			}
			else if (touchaction == "left"){
			 touchaction = "left";
			}
			else if (touchaction == "right"){
			 touchaction = "right";
			}
		   } else if (touchaction == "fire"){
			touchaction = "fire";
		   }
		   if (e.type == "touchend"){
			touchaction ="";
...
Ship = function () {
  ...
  this.preMove = function (delta) {
    if (KEY_STATUS.left || mouseaction == "left" || touchaction == "left") {
      this.vel.rot = -6;
    } else if (KEY_STATUS.right || mouseaction == "right" || touchaction == "right") {
      this.vel.rot = 6;
    } else {
      this.vel.rot = 0;
    }

    if (KEY_STATUS.up || mouseaction == "up" || touchaction == "up") {
    ...
    if (KEY_STATUS.space || mouseaction == "fire" || touchaction == "fire") {
      if (this.bulletCounter <= 0) {
...
BigAlien = function () {
  ...
  this.children.mustache = new Sprite();
  this.children.mustache.init("bigalien_top",
                            [-5, 0,
                          -4, -2,
                           5, -2,
                           6, 0]);
							
  this.children.mustache.visible = true;
...
/* AlienBullet Asteroid Functions */
...
Asteroid = function () {
  ...
  this.collidesWith = ["ship", "bullet", "asteroid"];
...
Game = {
  ...
  FSM: {
    ...
    waiting: function () {
	$("#back").hide();
      Text.renderText(window.ipad ? 'Touch Thrust to Start' : 'Press Thrust to Start', 20, Game.canvasWidth/2 - 150, Game.canvasHeight/2);
      if (KEY_STATUS.space || window.gameStart || mouseaction == "up" || touchaction == "up") {
    ...
    start: function () {
      ...
      Game.nextBigAlienTime = Date.now() + 30/*000*/ + (30/*000*/ * Math.random());
    ...
    run: function () {
      ...
      if (!Game.bigAlien.visible &&
        ...
        Game.nextBigAlienTime = Date.now() + (30/*000*/ * Math.random());
      }
    },
    ...
    end_game: function () {
      Text.renderText('GAME OVER', 47, Game.canvasWidth/2 - 160, Game.canvasHeight/2 + 5);
      if (this.timer == null) {
        this.timer = Date.now();
		var name = prompt("Please enter your name:  ", "");
		//localStorage.setItem("1", Game.score);
			
//Highscore Array Start	
function getHighScores() {
	var highScoresArray = localStorage.getItem("1");
		if (!highScoresArray) {
			highScoresArray = [];
				localStorage.setItem("1", JSON.stringify(highScoresArray));
			} else {
...
$(function () {
  ...
  var context = canvas[0].getContext("2d"); 1000/60;
